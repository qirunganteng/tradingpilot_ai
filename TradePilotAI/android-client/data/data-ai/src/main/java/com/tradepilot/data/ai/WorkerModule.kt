...
/**
 * Qualifier untuk OkHttpClient & WorkerApiService "milik" Worker Gateway,
 * dibedakan dari OkHttpClient default (:core-network, dipakai Gemini
 * langsung) yang tipenya sama persis (dulu: annotation class @WorkerRetrofit
 * ala Dagger @Qualifier).
 */
val WorkerRetrofitQualifier = named("WorkerRetrofit")
...
val dataAiWorkerModule = module {
     ...
     */
    single(BaseUrlInterceptorQualifier) {
    ...
    single(GatewayTokenInterceptorQualifier) {
        val secureKeyStore = get<SecureKeyStore>()
        Interceptor { chain ->
    ...
    single<OkHttpClient>(WorkerRetrofitQualifier) {
        val baseUrlInterceptor = get<Interceptor>(BaseUrlInterceptorQualifier)
        val gatewayTokenInterceptor = get<Interceptor>(GatewayTokenInterceptorQualifier)
    ...
    single<WorkerApiService>(WorkerRetrofitQualifier) {
        val client = get<OkHttpClient>(WorkerRetrofitQualifier)
        val moshi = get<Moshi>()
        Retrofit.Builder()
